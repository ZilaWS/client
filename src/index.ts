...
interface ICallableLocalEvents {
  ...
  /**
   * Runs when the client synced cookies with the server.
   * @returns
   */
  onCookieSync: () => void;
}
...
/**
 * Creates a new ZilaConnection.
 * You can connect to a ZilaWS server by calling the `.connectTo` function.
 * @example
 * const ws = new ZilaConnection();
 * ws.setMessageHandler("someEvent", (someParam: string) => {
 *  console.log(`Data from the server: ${someParam}`);
 * });
 *
 * await ws.connectTo("ws://127.0.0.1", (err) => {
 *  console.log(`An error happened with the connection: ${err}`);
 * });
 */
export class ZilaConnection {
  ...
  /**
   * Backwards-compatible factory for older consumers that used
   * `ZilaConnection.connectTo(...)` instead of `new ZilaConnection().connectTo(...)`.
   */
  public static connectTo(
    ...
    return new ZilaConnection().connectTo(wsUrl, errorCallback, allowSelfSignedCert);
  }
  ...
  public async connectTo(
    ...
    return new Promise<ZilaConnection>(async (resolve, reject) => {
      if (allowSelfSignedCert) process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";

      if (!wsUrl.includes("://")) {
        const error = new Error("Given string is not a URL: Missing protocol.");
        errorCallback?.(error.message);
        reject(error);
        return;
      }
      this.syncUrl = `${wsUrl.startsWith("wss") ? "https" : "http"}://${wsUrl.split("://")[1]}/zilaws/cookieSync`;
      ...
      try {
        ...
        if (this.isBrowser) {
          this.connection = new window.WebSocket(wsUrl);
      ...
      this.onceEventListener("onStatusChange", (status) => {
        if (this.connection && [WSStatus.CLOSED, WSStatus.ERROR].includes(status)) {
  ...
  /**
   * Gets the cookies from the server-side.
   *
   * **Attention!** The client can't send up cookies. If you wish to set a cookie to a client, do it from the server.
   * @returns If the sync was successfull
   * @throws If the [ServerUrl]/zilaws/cookiesync URL is not accessible
   */
  public async syncCookies() {
    const resp = (await fetch(this.syncUrl).catch((reason) => {
      throw new Error(`Could not sync cookies. Could not connect to ${this.syncUrl}. Reason: ${reason}`);
    })) as Response;
  ...
  private async callEventHandler(msg: string) {
    ...
    if (msgObj.identifier[0] != "@") {
      if (msgObj.identifier == "syncCookie") {
        if (!(await this.syncCookies())) console.error("Could not sync cookies");
      }
      return;
    }
    ...
    const callbackIdentifier = msgObj.identifier.slice(1);
    const callback = this.callbacks[callbackIdentifier];
...
/**
 * Backwards-compatible named export for older consumers.
 */
