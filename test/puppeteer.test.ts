 ...
 * Environment is provided by the Jest "browser" project preset (jest-puppeteer).
 * File auto-skips under Bun or plain Node (no global `page`).
 */

import { parse } from "cookie";
// Use the published/built server package rather than raw source path to avoid transform issues
import { ZilaServer } from "../../server/src";
import { connectTo } from "../src"; // Still used for non-browser connection tests

// Jest-Puppeteer exposes these globals at runtime; declare for TypeScript
...
// Increase default timeout for slower CI / headless startup
jest.setTimeout(30000);

(isPuppeteer ? describe : describe.skip)("ZilaWS Puppeteer integration", () => {
  // Lazy require inside block so Bun (when skipping) never evaluates these imports.
  ...
  it("connects client", async () => {
    ...
    await connectTo(`ws://127.0.0.1:${WS_PORT}`);
  }, 15_000);
