...
describe("Secure server tests", () => {
  ...
  beforeAll(async () => {
    return new Promise<void>(async (r) => {
      ...
      clientS = await connectTo("wss://127.0.0.1:6589", undefined, true);
      r();
    });
  });
  ...
  test.failing("Connecting error handler 2", async () => {
    await connectTo("ws://127.0.0.1:123", (err) => {}, true);
  });

  test.failing("Connecting error handler 3", async () => {
    await connectTo("wss://127.0.0.1:123", (err) => {}, true);
  });
  ...
  test("Simple Disconnect", async () => {
    const locClient = await connectTo("wss://127.0.0.1:6589", undefined, true);
  ...
  test("Kick", async () => {
    ...
    const locClient = await connectTo(
      "wss://127.0.0.1:6589",
      (reason) => {
  ...
  test("Client Already AsyncDisconnected", async () => {
    ...
    const locClient = await connectTo("wss://127.0.0.1:6591", undefined, true);
...
describe("WebSocket connection", () => {
  ...
  beforeAll(async () => {
    ...
    client = await connectTo("ws://127.0.0.1:6592", (reason?: string) => {
  ...
  test("Client Already AsyncDisconnected", async () => {
    ...
    const locClient = await connectTo("ws://127.0.0.1:6593");
